const CACHE='private-trip-v5-celtic-kickoff';
const VERSION='5';
const ASSETS=[
  './index.html',
...
const inject=html=>{
  let out=html
    ...
    .replace(/<link rel="stylesheet" href="\.\/theme\.css(?:\?v=[^"]*)?"\s*\/?>/g,'')
    .replace(/<script src="\.\/enhance\.js(?:\?v=[^"]*)?"><\/script>/g,'')
    .replace(/<link rel="icon" href="[^"]*"(?: type="[^"]*")?\s*\/?>/g,'')
    .replace(/<link rel="apple-touch-icon" href="[^"]*"\s*\/?>/g,'');
  out=out.replace('</head>','<link rel="stylesheet" href="./theme.css?v='+VERSION+'"><link rel="icon" href="./app-icon.svg" type="image/svg+xml"><link rel="apple-touch-icon" href="./app-icon.svg"></head>');
