...
const mission = [
  {
    ...
    title: "1. Supply & Demand Challenge",
    route: "/econ-arcade/supply-demand-lab",
    concepts: ["equilibrium", "surplus", "deadweight loss"],
  },
  {
    id: "prisoners-dilemma-arena",
    title: "2. Prisoner’s Dilemma",
    route: "/legacy-econ-arcade/prisoners-dilemma.html",
    concepts: ["repeated games", "cooperation", "retaliation"],
  },
  {
    id: "central-bank-simulator",
    title: "3. Central Banker",
    route: "/games/central-bank.html",
    concepts: ["inflation targeting", "policy lags", "credibility"],
  },
  {
    id: "invisible-hands",
    title: "4. Invisible Hands",
    route: "/econ-arcade/invisible-hands",
    concepts: ["systems thinking", "second-order effects", "trade retaliation"],
  },
] as const;
...
function migrateLegacyProgress() {
  ...
  if (!store.games["central-bank-simulator"]) {
    try {
      ...
      if (runs.length) {
        recordEconProgress("central-bank-simulator", {
          completed: true,
          countAttempt: false,
          score: runs[0]?.score,
          outcome: runs[0]?.ending || "Completed term",
          concepts: ["inflation targeting", "policy lags", "credibility", "financial stability"],
        });
      }
    } catch {
  ...
  if (!store.games["prisoners-dilemma-arena"] && localStorage.getItem("pd-seed-v3")) {
    recordEconProgress("prisoners-dilemma-arena", {
      countAttempt: false,
      outcome: "V3 match opened",
      concepts: ["repeated games", "cooperation", "retaliation", "continuation value"],
    });
  }
}
    ...
    <section className="rounded-3xl border border-emerald-300/25 bg-[linear-gradient(135deg,rgba(6,78,59,.22),rgba(15,23,42,.96),rgba(8,47,73,.2))] p-5 shadow-xl shadow-black/30 sm:p-6">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-300">Progress stays in this browser for now. Finish the four-game path to move from market mechanics to strategic interaction, macro policy, and full systems thinking.</p>
        </div>
        <Link href={next.route as Route} className="inline-flex min-h-12 items-center justify-center rounded-full bg-emerald-300 px-5 py-3 text-sm font-black uppercase tracking-[0.16em] text-slate-950">
          Play next: {next.title.replace(/^\d+\.\s*/, "")}
        </Link>
      </div>
        ...
        {mission.map((item) => {
          ...
          return (
            <Link key={item.id} href={item.route as Route} className={`rounded-2xl border p-4 transition hover:-translate-y-0.5 ${status === "completed" ? "border-emerald-300/40 bg-emerald-300/10" : status === "attempted" ? "border-amber-300/35 bg-amber-300/10" : "border-slate-700 bg-slate-950/60"}`}>
              <div className="flex items-start justify-between gap-2"><h3 className="font-bold text-white">{item.title}</h3><span className="rounded-full border border-current/25 px-2 py-1 text-[.62rem] font-bold uppercase text-slate-300">{status}</span></div>
      ...
      <div className="mt-5 rounded-2xl border border-cyan-300/20 bg-cyan-300/5 p-4">
        <p className="text-xs font-bold uppercase tracking-[0.2em] text-cyan-200">Concept mastery</p>
        <div className="mt-3 flex flex-wrap gap-2">
          {concepts.length ? concepts.map((concept) => <span key={concept} className="rounded-full border border-cyan-200/25 bg-cyan-200/10 px-3 py-1 text-xs text-cyan-50">{concept}</span>) : <span className="text-sm text-slate-400">Complete or attempt a core game to start building mastery badges.</span>}
        </div>
      </div>
    </section>
  );
