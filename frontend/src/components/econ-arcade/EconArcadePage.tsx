...
import { arcadeModules, curriculumPhases, playableArcadeModules, type ArcadeModule } from "@/lib/econ-arcade/curriculum";

function statusTone(status: string) {
  if (status === "available" || status === "playable") return "border-emerald-300/40 bg-emerald-300/10 text-emerald-100";
  if (status === "capstone" || status === "preview") return "border-cyan-300/40 bg-cyan-300/10 text-cyan-100";
...
function ModuleLaunchCard({ module, featured = false }: { module: ArcadeModule; featured?: boolean }) {
  ...
  const card = (
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="font-mono text-[0.65rem] font-black uppercase tracking-[0.18em] opacity-80">{module.format}</p>
          <h3 className="mt-2 text-xl font-black text-white">{module.title}</h3>
        </div>
        <span className="shrink-0 rounded-full border border-current/30 px-2 py-1 text-[0.65rem] font-black uppercase tracking-[0.16em]">{module.status}</span>
      </div>
      ...
      <p className="mt-4 rounded-xl border border-white/10 bg-black/20 p-3 text-xs leading-5 text-slate-300">
        <span className="font-bold text-white">Learn:</span> {module.learningGoal}
      </p>
  ...
  if (module.route) {
    return (
      <Link href={module.route as Route} className="block h-full focus:outline-none focus:ring-2 focus:ring-emerald-200 focus:ring-offset-2 focus:ring-offset-slate-950">
        {card}
      </Link>
    );
  }
  ...
  return (
    <a href={href} className="block h-full focus:outline-none focus:ring-2 focus:ring-emerald-200 focus:ring-offset-2 focus:ring-offset-slate-950">
      {card}
    </a>
  );
}
...
export function EconArcadePage() {
  ...
  const invisibleHands = playableArcadeModules.find((module) => module.id === "invisible-hands");
  ...
  return (
    <section className="space-y-6">
        ...
        <div className="grid gap-8 lg:grid-cols-[1.3fr_0.7fr] lg:items-center">
          <div>
            ...
            <p className="mt-5 max-w-3xl text-lg leading-8 text-slate-300">
              Econ Arcade turns economics into focused games, simulations, and strategic decision-making drills. Every playable module is surfaced here, including Invisible Hands, Central Banker, Prisoner’s Dilemma, Strategy Studio, and MacroBoard.
            </p>
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <Link
                href={(invisibleHands?.route ?? "/econ-arcade/supply-demand-lab") as Route}
            ...
            <div className="mt-5 grid gap-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="rounded-2xl border border-slate-800 bg-slate-900 p-4">
                  <p className="text-xs uppercase tracking-[0.2em] text-slate-500">Playable</p>
                  <p className="mt-2 font-mono text-3xl font-black text-white">{playableCount}</p>
                </div>
                <div className="rounded-2xl border border-slate-800 bg-slate-900 p-4">
      ...
      <section id="playable-games" aria-labelledby="playable-heading" className="space-y-4">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.28em] text-emerald-300">Playable launch bay</p>
            <h2 id="playable-heading" className="mt-2 text-2xl font-black text-white sm:text-3xl">All visible Econ games and labs</h2>
          </div>
      ...
      <section aria-labelledby="phase-heading" className="space-y-4">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          ...
          <p className="max-w-xl text-sm leading-6 text-slate-400">Playable modules now launch directly from the arcade while planned units remain clearly labeled as roadmap work.</p>
        </div>
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {curriculumPhases.map((phase) => (
            <article key={phase.id} className={`rounded-2xl border p-5 shadow-lg shadow-black/20 ${statusTone(phase.status)}`}>
              ...
              <p className="mt-2 text-xs uppercase tracking-[0.2em] opacity-80">{phase.status === "locked" ? "Roadmap" : `${phase.progress}% mapped`}</p>
            </article>
          ))}
        </div>
      </section>
      ...
      <section id="modules" aria-labelledby="module-heading" className="space-y-4">
        <p className="text-xs font-bold uppercase tracking-[0.28em] text-emerald-300">Complete module registry</p>
        <h2 id="module-heading" className="text-2xl font-black text-white sm:text-3xl">Playable, preview, and planned modules</h2>
      ...
      <section className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
        <article className="rounded-3xl border border-cyan-300/30 bg-cyan-300/10 p-6 shadow-xl shadow-black/30">
          <p className="font-mono text-xs font-black uppercase tracking-[0.3em] text-cyan-200">Featured systems simulator</p>
