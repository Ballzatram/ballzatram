...
export function EventTicker({ events, pending, turnPhase, resolved }: { events: GameEvent[]; pending: string[]; turnPhase:TurnPhase; resolved?: string[] }) {
  const resolvedFeed = resolved ?? ["Turn resolved.","Policy effects logged."];
  const feed = turnPhase === "resolved" ? resolvedFeed : turnPhase === "preview" ? ["Awaiting turn resolution.", ...pending] : events.map((e)=>e.title);
