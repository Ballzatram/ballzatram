import { globalNodeCards, globalRoutes } from "@/lib/invisible-hands-v2/data";
import type { Actor, GlobalPreviewEffect } from "@/lib/invisible-hands-v2/types";
...
export function GlobalTradeMap({ actors, previewEffect, selectedActorId, onSelect }: { actors: Actor[]; previewEffect?: GlobalPreviewEffect; selectedActorId?: string; onSelect:(id:string)=>void }) {
  const pos = Object.fromEntries(globalNodeCards.map((n) => [n.actorId ?? n.id, [n.x, n.y] as [number, number]]));
  const icon = Object.fromEntries(globalNodeCards.map((n) => [n.actorId ?? n.id, n.icon]));
  return <svg viewBox="0 0 100 100" className="h-full w-full">
    ...
    {globalRoutes.map((r,i)=>{const a=pos[r.from] as [number,number] | undefined;const b=pos[r.to] as [number,number] | undefined;if(!a||!b)return null;const active=!previewEffect||previewEffect.affectedRouteIds.includes(r.id);const color=kindColor[r.kind]??"#99b8d9";return <g key={r.id} opacity={active?0.95:0.12}><path d={path(a,b,i)} stroke={color} strokeWidth={active?0.58:0.24} strokeDasharray={["shipping","tariff","retaliation","sanction"].includes(r.kind)?"1.8 1.2":undefined} fill="none" markerEnd="url(#arrow)"/><text x={(a[0]+b[0])/2} y={(a[1]+b[1])/2-1.3} textAnchor="middle" fontSize="1.2" fill="#c6e9ff">{r.value ?? r.label}</text></g>;})}
    {actors.map((a)=>{const p=pos[a.id] as [number,number] | undefined;if(!p)return null;const selected=selectedActorId===a.id;const active=!previewEffect||previewEffect.affectedActorIds.includes(a.id)||a.id==="global-domestic";return <g key={a.id} onClick={()=>onSelect(a.id)} className="cursor-pointer" opacity={active?1:0.22}><rect x={p[0]-8.3} y={p[1]-5.4} width="16.6" height="10.8" rx="1.4" fill="#0f2744" stroke={selected?"#fde68a":"#5ac8ff"} strokeWidth={selected?0.6:0.25}/><text x={p[0]-7.3} y={p[1]-3.6} fontSize="1.7" fill="#def4ff">{icon[a.id] ?? "◉"}</text><text x={p[0]-4.8} y={p[1]-3.6} fontSize="1.18" fill="#e7f6ff">{a.name.slice(0,16).toUpperCase()}</text><text x={p[0]-7.3} y={p[1]-1.1} fontSize="0.98" fill="#84d4ff">STRESS {Math.round(a.stress)}</text><text x={p[0]-7.3} y={p[1]+1.2} fontSize="0.98" fill="#c6ebff">{a.id==="global-domestic"?"STATUS STABLE":"STATUS TENSE"}</text></g>;})}
