...
import { capstoneAccessTier, masteryTierRank, readEconProgress, recordEconProgress, type MasteryTier } from "@/lib/econ-arcade/progress";
...
export function InvisibleHandsPage() {
  ...
  const [capstoneAccess, setCapstoneAccess] = useState<MasteryTier>(() => capstoneAccessTier(readEconProgress()));
  ...
  function chooseTargetTier(tier: MasteryTier) {
    if (masteryTierRank[tier] <= masteryTierRank[capstoneAccess]) setTargetTier(tier);
  }
  ...
  useEffect(() => {
    ...
    const updated = recordEconProgress("invisible-hands", {
      completed: true,
      countAttempt: false,
    ...
    setCapstoneAccess(capstoneAccessTier(updated));
  ...
  const masteryDock = <MasteryDock targetTier={targetTier} setTargetTier={chooseTargetTier} accessTier={capstoneAccess} visitedLayers={visitedLayers} packageTurns={packageTurns} state={state} mastered={masteredScenarios} unlocked={unlockedScenarios} onScenario={changeScenario} />;

  if (state.layer !== "global") {
    return <>
      {masteryDock}
      <LayerCommandView state={state} scenarioName={scenario.name} scenarioObjective={scenario.objective} actors={layerActors} actions={layerActions} selectedActions={selectedActions} turnPhase={turnPhase} resolution={resolution} onToggleAction={toggleAction} onAdvance={onAdvance} onNextTurn={onNextTurn} onSetLayer={setLayer} onRestart={restart} onChangeScenario={changeScenario} unlockedScenarios={unlockedScenarios} targetTier={targetTier} visitedLayers={visitedLayers.size} packageTurns={packageTurns} />
    </>;
  }
