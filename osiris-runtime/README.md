...
- Text explanations and Parcel research use fresh ephemeral threads. Parcel alone enables hosted public web search and a server-owned research output schema; all other projects keep web search disabled. The health endpoint advertises `parcel-research-v1` for compatible Parcel clients. Shell, browser, image, plugin, file-viewing, memory, and multi-agent feature flags are disabled. The service rejects action approvals and exposes no generic RPC endpoint. It does not change project data.
...
4. In Parcel, save a brief and select Run research. Confirm progress and source-linked findings appear in the page, then add selected properties. For other configured projects, select a source or saved run, review the context, and explicitly send one short question. Confirm that the response stays on the project page and the request uses the signed-in account's allowance.
