...
def _prompt_words(prompt: str, max_words: int = 10) -> list[str]:
    words = str(prompt or "AI generated edit").replace("\n", " ").split()
    return words[:max_words] or ["AI", "generated", "edit"]


def _wrap_prompt_lines(prompt: str, words_per_line: int = 4, max_lines: int = 3) -> list[str]:
    words = _prompt_words(prompt, words_per_line * max_lines)
    lines = [" ".join(words[index:index + words_per_line]) for index in range(0, len(words), words_per_line)]
    return [line for line in lines[:max_lines] if line]


def _prompt_scene_filter(width: int, height: int, prompt: str, scene_title: str, overlays: list[dict], timeline_start: float, timeline_end: float, accent: str) -> str:
    filters = [
        "setsar=1",
        "fps=30",
        "format=yuv420p",
        f"drawbox=x=0:y=0:w=iw:h=ih:color={accent}@0.20:t=fill",
        "drawbox=x=70:y=120:w=940:h=1680:color=white@0.055:t=fill",
        "drawbox=x=70:y=120:w=940:h=1680:color=white@0.28:t=4",
        f"drawtext=text='{_escape_drawtext(scene_title)}':x=92:y=160:fontsize=44:fontcolor=white@0.84:borderw=2:bordercolor=black@0.7",
    ]
    y = 520
    for line in _wrap_prompt_lines(prompt):
        filters.append(
            "drawtext="
            f"text='{_escape_drawtext(line.upper())}':x=(w-text_w)/2:y={y}:fontsize=76:"
            "fontcolor=white:borderw=5:bordercolor=black@0.86:box=1:boxcolor=black@0.20:boxborderw=18"
        )
        y += 96
    filters.append(
        "drawtext=text='AI GENERATED BY BALLZATRAM':x=(w-text_w)/2:y=h-210:fontsize=38:"
        "fontcolor=white@0.76:borderw=3:bordercolor=black@0.72"
    )
    filters.extend(_studio_drawtext_filters(overlays, timeline_start, timeline_end))
    return ",".join(filters)


def render_prompt_video(plan: dict, output_path: str | Path, music_path: str | Path | None = None) -> Path:
    """Render an AI-planned prompt video into a vertical MP4 using local ffmpeg primitives.

    This creates a real MP4 even when the user has not uploaded source footage. The
    creative structure still comes from the AI edit plan; ffmpeg produces polished
    motion-card scenes and optionally mixes the user's rights-cleared music bed.
    """
    ensure_ffmpeg()
    output_path = Path(output_path)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    duration = max(6.0, min(float(plan.get("duration_seconds") or 18), 60.0))
    scene_count = 5 if duration >= 18 else 4
    scene_duration = duration / scene_count
    prompt = str(plan.get("visual_prompt") or plan.get("prompt") or "AI generated short-form edit")
    overlays = plan.get("text_overlays") or []
    accents = ["0x64d2ff", "0x9b7cff", "0x42f59e", "0xffd166", "0xff5d73"]

    with tempfile.TemporaryDirectory(prefix="ai-edit-prompt-video-") as temp_dir_name:
        temp_dir = Path(temp_dir_name)
        segment_paths: list[Path] = []
        timeline = 0.0
        for index in range(scene_count):
            segment_path = temp_dir / f"prompt_scene_{index:03d}.mp4"
            scene_title = f"SCENE {index + 1:02d} / AI PLAN"
            _run([
                "ffmpeg", "-y", "-hide_banner", "-loglevel", "error",
                "-f", "lavfi", "-i", f"color=c=0x060912:s={width}x{height}:r=30:d={scene_duration:.3f}",
                "-vf", _prompt_scene_filter(width, height, prompt, scene_title, overlays, timeline, timeline + scene_duration, accents[index % len(accents)]),
                "-an", "-c:v", "libx264", "-preset", "veryfast", "-crf", "23",
                "-movflags", "+faststart", str(segment_path),
            ])
            segment_paths.append(segment_path)
            timeline += scene_duration
        concat_file = temp_dir / "prompt_segments.txt"
        concat_file.write_text("".join(f"file '{path.as_posix()}'\n" for path in segment_paths), encoding="utf-8")
        stitched_path = temp_dir / "stitched_prompt_video.mp4"
        _run([
            "ffmpeg", "-y", "-hide_banner", "-loglevel", "error",
        ...
        if music:
            _run([
                ...
                "-t", f"{duration:.3f}", "-map", "1:v:0", "-map", "0:a:0",
