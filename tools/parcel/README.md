...
The published application is `/tools/parcel/`. It runs without a backend or model key. The older Next.js `/land` workspace remains available with its own saved projects and a link to the updated application. Its data, backend, and legacy pipeline outputs are not automatically imported into the new workspace.
...
**Research with my AI** opens the shared context-review panel with the saved brief, editable search areas, and only explicitly shortlisted properties. With no shortlist, it shares the brief alone. It never scans unrelated browser storage and makes no model request itself, even if an advanced runtime is configured.

The prepared prompt permits research only through browsing actually available in the user's AI app. It requires exact sources and unknown values for missing evidence. If browsing is unavailable, the model should return a search plan and explain the limitation. Results stay in that app until the user reviews and imports them here. No Parcel MCP tool, live listing feed, GIS intersection calculation, slope measurement, or routing API is connected.
...
- `assets/ai-features.js`: Parcel context contract. Only the app handoff permits available host browsing; the legacy text runtime still has no browsing or write capability.
...
Run `node --test scripts/parcel.test.cjs`, or `npm run test:static --prefix frontend`. The tests cover hard conflicts, unknown/stale evidence, criteria changes, lease/purchase budgets, malformed and duplicate imports, source preservation, safe text rendering, storage failures, and explicit AI context boundaries.
